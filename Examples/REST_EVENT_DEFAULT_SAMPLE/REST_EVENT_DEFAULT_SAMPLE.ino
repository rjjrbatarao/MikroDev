...
void eventLoop() {
  ...
  int eventType;
  char* event_args[5] = {"ev_0:", "ev_1:" , "ev_2:", "ev_3:" , "ev_4:"};
  dev.event(eventType, eventData, event_args);
  if (eventType == EVENT_ZERO) {
    Serial.print("Event0 ");
    Serial.println(eventData);
    freeMem();
  } else if (eventType == EVENT_ONE) {
    Serial.print("Event1 ");
    Serial.println(eventData);
    freeMem();
  } else if (eventType == EVENT_TWO) {
    Serial.print("Event2 ");
    Serial.println(eventData);
    freeMem();
  } else if (eventType == EVENT_THREE) {
    Serial.print("Event3 ");
    Serial.println(eventData);
    freeMem();
  } else if (eventType == EVENT_FOUR) {
    Serial.print("Event4 ");
    Serial.println(eventData);
    freeMem();
  }
